...
export default function UpdateBookingAfter({ Contract, setOpen }) {
  ...
  const handleUpdatePrice = async (data) => {
    ...
    try {
      const tx = await ContractFactory.updateBookingAfter(hoursInUnix)
      await tx.wait()
      setOpen(false)
    } catch (e) {
      console.error(e)
    ...
    <form onSubmit={handleSubmit(handleUpdatePrice)}>
      <div className="shadow sm:rounded-md">
        <div className="px-4 py-5 bg-white sm:p-6">
          <h2 className="py-2">Update Booking After</h2>
