...
export default function UpdateFactoryAddress({ Contract, setOpen }) {
  const { ContractDeploy } = useContextAPI();
  const { register, handleSubmit } = useForm();

  const handleUpdateFactoryAddress = async (data) => {
    try {
      const tx = await ContractDeploy.updateFactoryAddress(data.Address);
      await tx.wait();
      setOpen(false);
    } catch (e) {
      console.error(e);
  ...
  return (
    <form onSubmit={handleSubmit(handleUpdateFactoryAddress)}>
      <div className="shadow sm:rounded-md">
        <div className="px-4 py-5 bg-white sm:p-6">
          <h2 className="py-2">Update Factory Address</h2>
