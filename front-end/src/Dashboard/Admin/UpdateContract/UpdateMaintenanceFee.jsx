...
export default function UpdateMaintenanceFee({ Contract, setOpen }) {
  const { ContractFactory } = useContextAPI()
  const { register, handleSubmit } = useForm()

  const handleUpdatePrice = async (data) => {
    try {
      const tx = await ContractFactory.updateMaintenanceFee(
    ...
    <form onSubmit={handleSubmit(handleUpdatePrice)}>
      <div className="shadow sm:rounded-md">
        <div className="px-4 py-5 bg-white sm:p-6">
          <h2 className="py-2">Update Maintenance Fee</h2>
