...
export default function BookedDatesSidePanel({ open, setOpen, state }) {
  return (
                  ...
                  <div className="flex h-full flex-col overflow-y-scroll bg-white py-6 shadow-xl">
                    <div className="px-4 sm:px-6">
                      <Dialog.Title className="text-lg font-medium text-gray-900">
                        ...
                        Date: {state.data.date.day}/{state.data.date.month}/
                        {state.data.date.year}
                      </Dialog.Title>
                    </div>
                    <div className="relative mt-6 flex-1 px-4 sm:px-6">
                      {/* Replace with your content */}
                      <p className="mb-2">NFT #{state.data.contractinfo.id}</p>
                      <p className="mb-2">
                        Contract: {state.data.contractinfo.Contract.slice(0, 5)}
                        ...{state.data.contractinfo.Contract.slice(-4)}
                      </p>
                      <p className="mb-2">
                        Mobile Number: {state.data.mobileNumber}
                      </p>
                      <p className="mb-2">Food: {state.data.food}</p>
                      <br />
                      <p className="mb-2">Note: {state.data.note}</p>
